    ...
    for attack_type in attacks.keys():
        for attack_key in attacks[attack_type].keys():
            ...
            if os.path.exists(model_path):
                ...
                model = train_fn(model, train_data_loader, n_epochs=70, learning_rate=train_params["learning_rate"],
            ...
            #
            #     # 计算当前批次的预处理时间并存储
            #     batch_preprocess_time = batch_end_time - batch_start_time
            #     batch_preprocess_times.append(batch_preprocess_time)
            #
            # # 计算总的预处理时间
            # total_preprocess_time = sum(batch_preprocess_times)

            # 计算平均预处理时间
            # average_preprocess_time = total_preprocess_time / len(batch_preprocess_times)
            #
            # # 打印总的预处理时间和平均预处理时间
            # print(f"Total preprocessing time: {total_preprocess_time:.6f} seconds")
            # print(f"Average preprocessing time per batch: {average_preprocess_time:.6f} seconds")
