...
def set_normalize(model, normalize):
    if hasattr(model, 'normalize'):
        model.normalize = normalize
    elif hasattr(model, 'base_model') and hasattr(model.base_model, 'normalize'):
        model.base_model.normalize = normalize
    else:
        # 如果都没有，直接设置 normalize 属性
        print("警告：模型没有 'normalize' 属性或 'base_model'，将跳过设置 normalize")
        # 也可以在这里添加一些默认的处理逻辑
...
#         model.base_model.normalize = normalize
#     else:
#         print("Error: Model or model.base_model does not have a normalize attribute.")
...
def sabre_train(model, data_loader, n_epochs=100, learning_rate=1e-3, save_path=None, log_path=None, params={}):
    ...
    plot_images = params.get("plot_images", True)
    ...
    # 新增代码定义变量Variables to track loss and accuracy for plotting
    epoch_losses = []
    epoch_accuracies = []
        ...
        for i, (inputs, labels) in enumerate(data_loader, 0):
            ...
            if i % print_every == print_every - 1:
                ...
                if plot_images and epoch >= 70:
                    ...
                    #plt.show()
                    # 保存图像到指定文件夹，每个batch保存一张图像，命名为epoch_batch_编号
                    image_path = os.path.join(save_dir, f'epoch_{epoch}_batch_{i}_images.png')  # 完整路径
                    plt.savefig(image_path)
                    plt.close()  # 关闭图像，释放内存

        summarize_epoch(epoch, n_epochs, batch_count, running_losses, model, total, correct, log_path)

        # 新增代码Track loss and accuracy per epoch
        epoch_loss = running_losses['reconstruction'] + running_losses['classification']
    ...
    if save_path is not None:
        ...
        # 新增训练图Plot the training loss and accuracy
    plt.figure(figsize=(12, 6))

    # Plotting Loss
    plt.subplot(1, 2, 1)
    plt.plot(range(1, n_epochs + 1), epoch_losses, color='b', label='Training Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    ...
    # Plotting Accuracy
    plt.subplot(1, 2, 2)
    plt.plot(range(1, n_epochs + 1), epoch_accuracies, color='r', label='Training Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
...
#                 normalized_inputs = model.base_model.normalize_data(normalized_inputs)
#
#             denoised = model.preprocessing(inputs)
#
